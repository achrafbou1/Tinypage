...
export class UserService extends DatabaseService {
    ...
    /**
     * Gets a user by their stripe id.
     *
     * @param stripeId
     */
    async getUserByStripeId(stripeId: string): Promise<User> {
        let queryResult = await this.pool.query<DbUser>("select id, email_hash, full_name, active_profile_id, inventory, metadata, created_on from app.users where private_metadata->>'stripeId'=$1", [stripeId]);
     ...
     * @param stripeId
     */
    async getSensitiveUserByStripeId(stripeId: string): Promise<SensitiveUser> {
        let queryResult = await this.pool.query<DbSensitiveUser>("select id, email, email_hash, full_name, active_profile_id, inventory, metadata, created_on, private_metadata from app.users where private_metadata->>'stripeId'=$1", [stripeId]);
    ...
    async sendPasswordResetEmail(email: string): Promise<User> {
        ...
        let urlString = config.editorUrl + "/forgot-password/change?";
    ...
    async createUserWithGoogle(email: string, googleId: string, name?: string): Promise<SensitiveUser> {
        ...
        let enableGoogleSignInQueryResult = await this.pool.query<{ google_id: string }>("update app.users set private_metadata = jsonb_set(private_metadata::jsonb, '{googleId}', $1, true) where email=$2 returning private_metadata->>'googleId' as google_id",
            [
    ...
    async setEmailNotifications(userId: string, emailNotifications: DbSensitiveUser["private_metadata"]["emailNotifications"]): Promise<SensitiveUser> {
        let profileQuery = await this.pool.query<DbSensitiveUser>(`update app.users
                                                                   ...
                                                                   where id = $2
                                                                   returning *;`,
            [JSON.stringify(emailNotifications), userId]);
